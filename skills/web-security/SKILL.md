...
Report severity, affected path/endpoint, preconditions, observed impact, fix, and test evidence. Mark unverified deployment assumptions explicitly. Use `code-review` for a broader diff review and `babysit` only when the user requests PR follow-through. Do not claim a complete penetration test or compliance certification from this workflow.
