...
Isolate test ports, databases, generated output directories, and writable caches when tests use shared resources. Worktrees alone do not isolate external services. When introducing concurrent service setup, use [setup and runtime contracts](../ci-maintenance/references/setup-contracts.md#isolate-concurrent-checkouts-and-services) for checkout identities, port allocation, readiness, and owned cleanup. Check repository support before using linked worktrees with submodules. Use a separate clone or serial work when the layout or tooling cannot support them.
