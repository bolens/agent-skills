...
- For GSAP, Motion, native transitions, and other interface animation, use `web-animation` for runtime-specific sampling, interruption, and teardown checks. Use `animation-assets` for Lottie/Rive loading and state-machine evidence. Animation-disabled screenshots cannot establish playback behavior or frame cost.
